import type { Task, Logger } from '@ai-tooling/shared';
...
export interface SchedulerConfig { tickIntervalMs?: number; defaultTimeoutMs?: number; defaultTokenBudget?: number; logger?: Logger; }
export interface Scheduler { tick(): Promise<void>; start(): () => void; }
export function createScheduler(taskRepo: TaskRepository, executionRepo: ExecutionRepository, concurrency: ConcurrencyTracker, agents: AgentRuntime[], config: SchedulerConfig = {}): Scheduler {
  ...
  async function dispatchTask(task: Task): Promise<void> {
    ...
    if (capable.length === 0) { log.warn('No capable agents for task', { taskId: task.id }); return; }
    if (!concurrency.canRun(task.projectId)) { return; }
    ...
    setTimeout(async () => {
      try {
        ...
        if (result.ok) { await executionRepo.complete(execution.id); await queueEngine.markSuccess(task.id); breaker.recordSuccess(); }
        else { breaker.recordFailure(); const { shouldRetry } = classifyFailure(result.error, task.retryCount, task.maxRetries); if (shouldRetry) { await executionRepo.fail(execution.id); await taskRepo.incrementRetry(task.id); await taskRepo.updateStatus(task.id, 'pending'); } else { await executionRepo.fail(execution.id); await queueEngine.markFailed(task.id, result.error); } }
      } catch (e) {
        ...
        await executionRepo.fail(execution.id); await queueEngine.markFailed(task.id, error); breaker.recordFailure();
      } finally { concurrency.release(task.projectId); }
    }, 0);
  }
  const scheduler: Scheduler = {
    async tick() { if (!breaker.canCall()) return; const pending = await taskRepo.findPendingWithPrompt(); for (const task of pending) { await dispatchTask(task); } },
    start() {
      let running = true; let timer: ReturnType<typeof setTimeout>;
      async function loop() { if (!running) return; await scheduler.tick(); if (running) timer = setTimeout(loop, tickIntervalMs); }
      void scheduler.tick(); timer = setTimeout(loop, tickIntervalMs);
